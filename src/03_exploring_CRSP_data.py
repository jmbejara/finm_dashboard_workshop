"""
In this script, I want to do some basic exploration of the new CRSP CIZ data.
"""
...
# To find the right table, let's use a combination of the web query interface
# and the SAS Studio explorer.
# Let's start by looking at the web query interface:
...
## Doesn't work:
# This is because the web queries usual use merges of tables from
# many different tables. The results of the merge are not usually
# available through the Python API interfact. Often, you'll have
# to merge the tables yourself.

# df = db.get_table(library="crspa", table="wrds_dsfv2_query", obs=10)

## However, in this case, we can use the SAS Studio explorer to find the right table.
# And, lucky for us, the data in the web query is available in a pre-merged
# table available through the Python API. But, first, let's look at the
# standard daily stock file (DSF) from the  CIZ format.
...
df.head()
# Now that we've done this, notice that we actually just made a mistake above.
# For some reason, we aren't allowed to access this via "crspa", but we
# are via "crsp".
df.info()
# Notice that this now matches the web query variables list.

## Now, let's explore some of the columns. But first, let's download more data.
query = """
SELECT 
    permno, 
    permco, 
